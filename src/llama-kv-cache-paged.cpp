...
bool llama_kv_cache_paged::seq_rm(
    ...
    if (p0 == 0 || p0 <= meta.pos_min) {
        ...
        if (debug > 0) {
            fprintf(stderr, "%s: removed entire sequence %d (%zu blocks freed)\n",
                    __func__, seq_id, blocks.size());
        }
    ...
    fprintf(stderr, "%s: partial sequence removal (p0=%d, p1=%d) not yet supported in paged cache\n",
            __func__, p0, p1);
    return false;
}
